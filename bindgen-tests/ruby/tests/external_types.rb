...
class TestExternalTypes < Test::Unit::TestCase
  ...
  def test_ext_nested_rec
    rec = Rich::ExternalNestedRec.new(
      en: Rich::ExternalEnum::ONE,
      rec: Rich::ExternalRec.new(a: 7)
    )
    result = Api.roundtrip_ext_nested_rec(rec)

    assert_equal Rich::ExternalEnum::ONE, result.en
    assert_equal 7, result.rec.a
  end

  def test_nested_ext_rec_identity_custom
    inner = Rich::ExternalRec.new(a: 9)
    result = Api.roundtrip_nested_ext_rec(inner)

    assert_instance_of Rich::ExternalRec, result
    assert_equal 9, result.a
  end

  def test_nested_ext_interface_identity_custom
    obj = Rich::ExternalInterface.new(3)
    result = Api.roundtrip_nested_ext_interface(obj)

    assert_instance_of Rich::ExternalInterface, result
    assert_equal 3, result.get_value
  end

  def test_optional_and_sequence
    assert_nil Api.roundtrip_maybe_ext_enum(nil)
    assert_equal Rich::ExternalEnum::TWO,
                 Api.roundtrip_maybe_ext_enum(Rich::ExternalEnum::TWO)
    assert_equal [Rich::ExternalEnum::ONE, Rich::ExternalEnum::THREE],
                 Api.roundtrip_ext_enums(
                   [Rich::ExternalEnum::ONE, Rich::ExternalEnum::THREE]
                 )
  end

  def test_async_ext_enum
    result = Api.async_roundtrip_ext_enum(Rich::ExternalEnum::TWO)
    assert_equal Rich::ExternalEnum::TWO, result
  end
  ...
  def test_mid_rec_roundtrip
    rec = Mid::MidRec.new(
      inner: Rich::ExternalRec.new(a: 11),
      maybe_enum: Rich::ExternalEnum::ONE
    )
    result = Api.roundtrip_mid_rec(rec)

    assert_equal 11, result.inner.a
    assert_equal Rich::ExternalEnum::ONE, result.maybe_enum
  end
