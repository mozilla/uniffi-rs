...
Binding generation can be configured with a `uniffi.toml` in the root of each crate,
and by specifying the path to a toml file which specifies global configuration.

For the builtin bindings, see [Kotlin](./kotlin/configuration.md), [Python](./python/configuration.md) and [Swift](./swift/configuration.md).
