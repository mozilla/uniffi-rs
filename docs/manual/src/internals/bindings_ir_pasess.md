...
Steps are typically implemented using the `Node::visit_mut()` function to add new field data.
For example, the step to handle the module_path -> module name transformation could look like this:

```rust
pub fn step(root: &mut pass:Root) -> Result<()> {
    root.visit_mut(|ty: &mut Type| {
        match ty {
...
Note: module_name normalization is actually done as part of a larger step that moves modules into a tree.
...
For example the `self_types` pass adds a `self_type` field to type definitions, which will allow them to do things like check the `self_type.is_used_as_error` field.
This is handled as follows:

```rust
pub fn step(root: &mut Root) -> Result<()> {
    // Visit the parent module first, so we can get the module name to use
    root.visit_mut(|module: &mut Module| {
        let module_name = module.name.clone();

        // Then visit each `Interface` and set the `self_type` field.
