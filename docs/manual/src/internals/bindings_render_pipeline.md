...
This phase transforms the general `BindingsIr` into a language-specific one:
...
* As bindings generators walk the IR tree to do these transforms, they can also generate new language-specific IR items, like import statements.


When the function IR is specialized for Python, 

When the `a` argument from the `BindingsIr` is specialized for Python, the `lang_data` fields are populated with:
  * Python type names
  * FFIConverter class names

Other languages would also transform function and argument names into the idiomatic naming style, but Python and Rust happen to line up.
