...
To work around this, UniFFI adds an extra box creating a `Box<Arc<dyn Trait>>` value.
This effectively converts the wide pointer into a regular pointer, since `Arc<dyn Trait>` is sized (at 2 words).
Finally `Box::into_raw` is called to create a raw pointer, which is converted into a `u64`.
Other than this extra layer of boxing, everything works the same as regular interfaces.
