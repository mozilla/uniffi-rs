...
Be sure to pass a pass `path-to-library-file` or `src:[crate-name]` when generating bindings for crates that use UniFFI proc-macros. Passing a path to a UDL file won't work. See the [Foreign language bindings docs](../tutorial/foreign_language_bindings.md) for more info.
