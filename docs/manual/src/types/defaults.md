...
You can specify that you want either the "natural" default for the type, or you can specify a literal.
...
Here we can see that `d` will use the "natural default" (ie, `0`), but `e` will use the literal value for the default.
These args can be omitted when creating the `Enum` in the foreign bindings.

Note that UDL only supports literal values.
