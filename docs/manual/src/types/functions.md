...
Both [UDL](../udl/functions.md) and [proc-macros](../proc_macro/functions.md) allow you to specify a default value for function arguments.

While the Rust code will still take a required argument:
...
Async functions "just work" with proc-macros, while UDL can use the `[Async]` attribute:
