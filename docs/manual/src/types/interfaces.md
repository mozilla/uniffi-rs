...
Interfaces are passed by reference so can not have data items - unlike a [Dictionary](./structs.md) or Enum, which are passed by value so *only* have data fields and no methods.
...
Note [UDL](../udl/interfaces.md) requires a `Trait` attribute; proc-macros `#[uniffi::export]` the trait declaration.
