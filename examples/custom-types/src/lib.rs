...
uniffi::custom_type!(TimeIntervalSecDbl, f64, {
    into_bridge_type: |time_interval| time_interval.0,
    try_from_bridge_type: |val| Ok(TimeIntervalSecDbl(val)),
});
...
uniffi::custom_type!(Url, String, {
    try_from_bridge_type: |val| Ok(Url::parse(&val)?),
    into_bridge_type: |obj| obj.into(),
});
