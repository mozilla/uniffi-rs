...
uniffi::custom_type!(TimeIntervalSecDbl, f64, {
    into_existing: |time_interval| time_interval.0,
    try_from_existing: |val| Ok(TimeIntervalSecDbl(val)),
});
...
uniffi::custom_type!(Url, String, {
    try_from_existing: |val| Ok(Url::parse(&val)?),
    into_existing: |obj| obj.into(),
});
