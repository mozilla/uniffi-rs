...
pub unsafe fn decode_jni_string(env: *mut JNIEnv, string: jstring) -> Result<String> {
    ...
    unsafe {
        let len = ((**env).v1_4.GetStringUTFLength)(env, string);
        let data = ((**env).v1_4.GetStringUTFChars)(env, string, std::ptr::null_mut());
        if data.is_null() {
            bail!("UniFFI internal error in decode_jni_string: GetStringUTFChars returned NULL");
        }
        ...
        ((**env).v1_4.ReleaseStringUTFChars)(env, string, data);
        Ok(rust_string?)
    }
}
...
impl JniString {
    ...
    pub unsafe fn into_jstring(&self, env: *mut JNIEnv) -> jstring {
        ...
        unsafe { ((**env).v1_4.NewStringUTF)(env, self.as_ptr()) }
    }
}
