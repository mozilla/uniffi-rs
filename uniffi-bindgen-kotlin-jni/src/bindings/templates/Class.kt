...
open class {{ cls.name_kt() }} : {{ cls.base_classes|join(", ") }} {
     ...
     * @suppress
     */
    constructor(withHandle: uniffi.WithHandle, uniffiHandle: kotlin.Long) {
    ...
    constructor({{ cons.callable.arg_list_kt() }}) {
     ...
     * @suppress
     */
    public val uniffiHandle: kotlin.Long
     ...
     */
    val uniffiIsDestroyed: kotlin.Boolean get() = wasDestroyed.get()
    ...
    private class UniffiCleanAction(private val uniffiHandle: kotlin.Long) : Runnable {
        override fun run() {
     ...
     * @suppress
     */
    fun uniffiCloneHandle(): kotlin.Long {
        if (uniffiHandle == 0.toLong()) {
            throw uniffi.InternalException("uniffiCloneHandle() called on NoHandle object");
        }
        return uniffi.Scaffolding.{{ cls.jni_clone_name() }}(uniffiHandle)
    }

    {% for meth in cls.methods -%}
    override public fun {{ meth.callable.name_kt() }}({{ meth.callable.arg_list_kt() }}): {{ meth.callable.return_type_kt() }} {
    ...
    companion object {
        ...
        public fun {{ cons.callable.name_kt() }}({{ cons.callable.arg_list_kt() }}): {{ cons.callable.return_type_kt() }} {
            {%- let jni_method_name = cons.jni_method_name %}
            {%- let callable = cons.callable %}
            {% filter indent(8) %}{%- include "CallableBody.kt" %}{% endfilter %}
        }
