...
pub fn run_foreign_language_testcase(
    ...
    run_uniffi_bindgen_test(out_dir, interface_file, test_file)?;
    Ok(())
}
...
fn run_uniffi_bindgen_test(out_dir: &str, interface_file: &str, test_file: &str) -> Result<()> {
    let status = Command::new("uniffi-bindgen")
        .args(&["test", out_dir, interface_file, test_file])
        .status()?;
    if !status.success() {
...
fn run_uniffi_bindgen_test(out_dir: &str, interface_file: &str, test_file: &str) -> Result<()> {
    uniffi_bindgen::run_tests(out_dir, interface_file, vec![test_file], None)
}
