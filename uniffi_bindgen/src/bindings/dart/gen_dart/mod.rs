...
/// Config options for the caller to customize the generated Dart.
///
/// Note that this can only be used to control details of the Dart *that do not affect the underlying component*,
...
impl Config {
    /// The name of the Dart module containing the high-level foreign-language bindings.
    ...
    /// Whether to omit argument labels in Dart function definitions.
...
/// Generate UniFFI component bindings for Dart, as strings in memory.
///
pub fn generate_bindings(config: &Config, ci: &ComponentInterface) -> Result<Bindings> {
    let header = BridgingHeader::new(config, ci)
        .render()
        .context("failed to render Dart bridging header")?;
    let library = DartWrapper::new(config.clone(), ci)
        .render()
        .context("failed to render Dart library")?;
    let modulemap = if config.generate_module_map() {
        Some(
            ModuleMap::new(config, ci)
                .render()
                .context("failed to render Dart modulemap")?,
        )
    } else {
        None
    };
...
/// Renders Dart helper code for all types
///
...
impl<'a> TypeRenderer<'a> {
    ...
    // The following methods are used by the `Types.dart` macros.
...
/// as a Dart module that can be called by other Dart code. In our case, its only
/// job is to define the *name* of the Dart module that will contain the FFI functions
/// so that it can be imported by the higher-level code in from [`DartWrapper`].
#[derive(Template)]
...
impl<'a> DartWrapper<'a> {
    ...
    pub fn initialization_fns(&self) -> Vec<String> {
        self.ci
            .iter_types()
            .into_iter()
            .filter_map(|t| t.initialization_fn(&DartCodeOracle))
            .collect()
    }
}
...
impl DartCodeOracle {
    ...
    //
    // There is a companion match in `templates/Types.dart` which performs a similar function for the
    // template code.
    //
    //   - When adding additional types here, make sure to also add a match arm to the `Types.dart` template.
...
impl CodeOracle for DartCodeOracle {
...
pub mod filters {
    ...
    fn oracle() -> &'static DartCodeOracle {
        &DartCodeOracle
    }
    ...
    /// Get the Dart syntax for representing a given low-level `FfiType`.
    ...
    /// Get the idiomatic Dart rendering of an individual enum variant.
    pub fn enum_variant_dart(nm: &str) -> Result<String, askama::Error> {
