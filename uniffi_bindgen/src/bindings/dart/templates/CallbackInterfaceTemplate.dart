...
public protocol {{ type_name }} : AnyObject {
    ...
    func {{ meth.name()|fn_name }}({% call dart::arg_list_protocol(meth) %}) {% call dart::throws(meth) -%}
...
fileprivate let {{ foreign_callback }} : ForeignCallback =
    ...
    func {{ method_name }}(_ dartCallbackInterface: {{ type_name }}, _ argsData: UnsafePointer<UInt8>, _ argsLen: Int32, _ out_buf: UnsafeMutablePointer<RustBuffer>) throws -> Int32 {
        ...
        func makeCall() throws -> Int32 {
            let result = try dartCallbackInterface.{{ meth.name()|fn_name }}(
        ...
        {%- when None %}
        func makeCall() throws -> Int32 {
            try dartCallbackInterface.{{ meth.name()|fn_name }}(
                    {% for arg in meth.arguments() -%}
                    {% if !config.omit_argument_labels() %}{{ arg.name()|var_name }}: {% endif %} try {{ arg|read_fn }}(from: &reader)
                    {%- if !loop.last %}, {% endif %}
                    {% endfor -%}
...
extension {{ ffi_converter_name }} : FfiConverter {
    ...
    public static func lift(_ handle: UniFFICallbackHandle) throws -> DartType {
    ...
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> DartType {
        ensureCallbackinitialized();
        let handle: UniFFICallbackHandle = try readInt(&buf)
        return try lift(handle)
    }

    public static func lower(_ v: DartType) -> UniFFICallbackHandle {
        ensureCallbackinitialized();
        return handleMap.insert(obj: v)
    }

    public static func write(_ v: DartType, into buf: inout [UInt8]) {
        ensureCallbackinitialized();
