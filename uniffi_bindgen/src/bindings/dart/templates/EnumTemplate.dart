...
public enum {{ type_name }} {
    ...
    case {{ variant.name()|enum_variant_dart }}{% if variant.fields().len() > 0 %}({% call dart::field_list_decl(variant) %}){% endif -%}
    {% endfor %}
}
...
public struct {{ ffi_converter_name }}: FfiConverterRustBuffer {
    ...
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> {{ type_name }} {
        ...
        switch variant {
        {% for variant in e.variants() %}
        case {{ loop.index }}: return .{{ variant.name()|enum_variant_dart }}{% if variant.has_fields() %}(
    ...
    public static func write(_ value: {{ type_name }}, into buf: inout [UInt8]) {
        ...
        case let .{{ variant.name()|enum_variant_dart }}({% for field in variant.fields() %}{{ field.name()|var_name }}{%- if loop.last -%}{%- else -%},{%- endif -%}{% endfor %}):
            writeInt(&buf, Int32({{ loop.index }}))
            {% for field in variant.fields() -%}
            {{ field|write_fn }}({{ field.name()|var_name }}, into: &buf)
            {% endfor -%}
        {% else %}
        case .{{ variant.name()|enum_variant_dart }}:
            writeInt(&buf, Int32({{ loop.index }}))
        {% endif %}
        {%- endfor %}
        }
