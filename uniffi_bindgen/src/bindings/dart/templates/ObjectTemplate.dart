...
public protocol {{ obj.name() }}Protocol {
    ...
    func {{ meth.name()|fn_name }}({% call dart::arg_list_protocol(meth) %}) {% call dart::async(meth) %} {% call dart::throws(meth) -%}
...
public class {{ type_name }}: {{ obj.name() }}Protocol {
    ...
    // TODO: We'd like this to be `private` but for Darty reasons,
    ...
    {%- when Some with (cons) %}
    public convenience init({% call dart::arg_list_decl(cons) -%}) {% call dart::throws(cons) %} {
        self.init(unsafeFromRawPointer: {% call dart::to_ffi_call(cons) %})
    }
    {%- when None %}
    {%- endmatch %}
    ...
    public static func {{ cons.name()|fn_name }}({% call dart::arg_list_decl(cons) %}) {% call dart::throws(cons) %} -> {{ type_name }} {
    ...
    public func {{ meth.name()|fn_name }}({%- call dart::arg_list_decl(meth) -%}) async {% call dart::throws(meth) %}{% match meth.return_type() %}{% when Some with (return_type) %} -> {{ return_type|type_name }}{% when None %}{% endmatch %} {
        let future = {% call dart::to_ffi_call_with_prefix("self.pointer", meth) %}
    ...
    public func {{ meth.name()|fn_name }}({% call dart::arg_list_decl(meth) %}) {% call dart::throws(meth) %} -> {{ return_type|type_name }} {
        return {% call dart::try(meth) %} {{ return_type|lift_fn }}(
            {% call dart::to_ffi_call_with_prefix("self.pointer", meth) %}
        )
    }

    {%- when None %}

    public func {{ meth.name()|fn_name }}({% call dart::arg_list_decl(meth) %}) {% call dart::throws(meth) %} {
        {% call dart::to_ffi_call_with_prefix("self.pointer", meth) %}
    }
