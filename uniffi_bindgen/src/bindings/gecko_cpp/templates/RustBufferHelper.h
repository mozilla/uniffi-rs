...
struct ViaFfi<bool, uint8_t> {
  [[nodiscard]] static bool Lift(const uint8_t& aLowered, bool& aLifted) {
    aLifted = aLowered != 0;
    return true;
  }
  [[nodiscard]] static uint8_t Lower(const bool& aLifted) {
    return aLifted ? 1 : 0;
  }
};
...
template <typename T>
struct Serializable<Maybe<T>> {
  static CheckedInt<size_t> Size(const Maybe<T>& aValue) {
    if (!aValue.isSome()) {
      return 1;
    }
    CheckedInt<size_t> size(1);
    size += Serializable<T>::Size(aValue.ref());
    return size;
  }

  [[nodiscard]] static bool ReadFrom(Reader& aReader, Maybe<T>& aValue) {
    ...
    aValue = Maybe<T>(std::move(value));
    return true;
  };

  static void WriteInto(Writer& aWriter, const Maybe<T>& aValue) {
    if (!aValue.isSome()) {
      aWriter.WriteUInt8(0);
    } else {
      aWriter.WriteUInt8(1);
      Serializable<T>::WriteInto(aWriter, aValue.ref());
    }
  }
};
...
struct Serializable<HashMap<K, V>> {
  static CheckedInt<size_t> Size(const HashMap<K, V>& aValue) {
    CheckedInt<size_t> size;
    size += sizeof(int32_t);  // For the length prefix.
    for (auto iter = aValue.iter(); !iter.done(); iter.next()) {
      size += Serializable<K>::Size(iter.get().key());
      size += Serializable<V>::Size(iter.get().value());
    }
    return size;
  }

  [[nodiscard]] static bool ReadFrom(Reader& aReader, HashMap<K, V>& aValue) {
    uint32_t length = aReader.ReadUInt32();
    if (!aValue.reserve(static_cast<uint32_t>(length))) {
      return false;
    }
    for (uint32_t i = 0; i < length; ++i) {
      typename HashMap<K, V>::Entry::KeyType key;
      if (!Serializable<K>::ReadFrom(aReader, key)) {
        return false;
      }
      typename HashMap<K, V>::Entry::ValueType value;
      if (!Serializable<V>::ReadFrom(aReader, value)) {
        return false;
      }
      aValue.putNewInfallible(std::move(key), std::move(value));
    }
    return true;
  };

  static void WriteInto(Writer& aWriter, const HashMap<K, V>& aValue) {
    aWriter.WriteUInt32(aValue.count());
    for (auto iter = aValue.iter(); !iter.done(); iter.next()) {
      Serializable<K>::WriteInto(aWriter, iter.get().key());
      Serializable<V>::WriteInto(aWriter, iter.get().value());
    }
  }
};
