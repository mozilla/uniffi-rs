...
sealed class {{ type_name }}{% if contains_object_references %}: Disposable {% endif %} {
    ...
    data class {{ variant|enum_variant|type_name }}(
        ...
        val {{ field.name()|var_name }}: {{ field.type_()|protocol_name}}{% if loop.last %}{% else %}, {% endif %}
        {% endfor -%}
    ) : {{ type_name }}() {
        companion object
    }
    ...
    override fun allocationSize(value: {{ type_name }}) = when(value) {
        ...
        is {{ type_name }}.{{ variant|enum_variant|type_name }} -> {
            ...
            (
                4
                {%- for field in variant.fields() %}
                + {{ field|allocation_size_fn }}(value.{{ field.name()|var_name }}{{ field.type_()|downcast_if_needed }})
                {%- endfor %}
            )
        }
        {%- endfor %}
    ...
    override fun write(value: {{ type_name }}, buf: ByteBuffer) {
        when(value) {
            ...
            is {{ type_name }}.{{ variant|enum_variant|type_name }} -> {
                ...
                {%- for field in variant.fields() %}
                {{ field|write_fn }}(value.{{ field.name()|var_name }}{{ field.type_()|downcast_if_needed }}, buf)
                {%- endfor %}
                Unit
            }
            {%- endfor %}
