...
class {{ impl_class_name }}(
    ...
    override suspend fun {{ meth.name()|fn_name }}({%- call kt::arg_list_decl(meth) -%}){% match meth.return_type() %}{% when Some with (return_type) %} : {{ return_type|protocol_name }}{% when None %}{%- endmatch %} {
    ...
    override fun {{ meth.name()|fn_name }}({% call kt::arg_list_protocol(meth) %}): {{ return_type|protocol_name }} =
        callWithPointer {
