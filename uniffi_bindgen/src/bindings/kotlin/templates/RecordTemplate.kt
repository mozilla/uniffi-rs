...
data class {{ type_name }} (
    ...
    {% if config.generate_immutable_records() %}val{% else %}var{% endif %} {{ field.name()|var_name(config) }}: {{ field|type_name(ci, config) -}}
...
public object {{ rec|ffi_converter_name }}: FfiConverterRustBuffer<{{ type_name }}> {
    ...
    override fun allocationSize(value: {{ type_name }}) = {%- if rec.has_fields() %} (
        {%- for field in rec.fields() %}
            {{ field|allocation_size_fn }}(value.{{ field.name()|var_name(config) }}){% if !loop.last %} +{% endif %}
        {%- endfor %}
    ) {%- else %} 0UL {%- endif %}
