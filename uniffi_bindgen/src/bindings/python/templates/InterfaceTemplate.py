...
{%-         let callable = eq.callable %}
    def __eq__(self, other: object) -> {{ callable.return_type.type_name }}:
        if not isinstance(other, {{ int|type_name }}):
...
{%- let ffi_converter_name = int|ffi_converter_name %}
...
class {{ int|ffi_converter_name }}__as_error(_UniffiConverterRustBuffer):
    @classmethod
    def read(cls, buf):
    ...
    def lift(value):
        ...
        with value.consume_with_stream() as stream:
            return {{ int|ffi_converter_name }}.read(stream)
...
class {{ int|ffi_converter_name }}:
    ...
    @staticmethod
    def check_lower(value: {{ int|type_name }}):
        {%- if int.vtable.is_some() %}
        pass
        {%- else %}
    ...
    def lower(value: {{ protocol.name }}):
        {%- if int.vtable.is_some() %}
        return {{ int|ffi_converter_name }}._handle_map.insert(value)
        {%- else %}
        if not isinstance(value, {{ int.name }}):
            raise TypeError("Expected {{ int.name }} instance, {} found".format(type(value).__name__))
