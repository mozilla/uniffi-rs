...
def _uniffi_check_contract_api_version(lib):
    ...
    scaffolding_contract_version = lib.{{ globals.ffi_uniffi_contract_version.name }}()
    if bindings_contract_version != scaffolding_contract_version:
        raise InternalError("UniFFI contract version mismatch: try cleaning and rebuilding your project")

def _uniffi_check_api_checksums(lib):
    {%- for check in checksum_checks %}
    if lib.{{ check.func.name }}() != {{ check.checksum }}:
...
class {{ ffi_struct.name }}(ctypes.Structure):
    _fields_ = [
        ...
        ("{{ field.name }}", {{ field|ffi_type_name }}),
        {%- endfor %}
    ]
...
_UniffiLib.{{ func.name }}.restype = {{ func|ffi_return_type }}
{%- endmatch %}
{%- endfor %}
