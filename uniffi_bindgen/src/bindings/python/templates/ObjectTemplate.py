...
class {{ interface.name }}({{ interface|base_classes }}):
    ...
    def __del__(self):
        ...
        if pointer is not None:
            _uniffi_rust_call(_UniffiLib.{{ interface.ffi_free.name }}, pointer)

    def _uniffi_clone_pointer(self):
        return _uniffi_rust_call(_UniffiLib.{{ interface.ffi_clone.name }}, self._pointer)
...
{# Special-case these to return NotImplemented when the wrong type is passed in #}
    def __eq__(self, other: object) -> bool:
        ...
        return {{ eq.return_type.ty.as_ref().unwrap()|lift_fn }}(
            _uniffi_rust_call_with_error(
                None,
                _UniffiLib.{{ eq.ffi_func().name }},
    ...
    def __ne__(self, other: object) -> bool:
        ...
        return {{ ne.return_type.ty.as_ref().unwrap()|lift_fn }}(
            _uniffi_rust_call_with_error(
                None,
                _UniffiLib.{{ ne.ffi_func().name }},
                self._uniffi_clone_pointer(),
                {{ interface|lower_fn }}(other),
            )
        )
