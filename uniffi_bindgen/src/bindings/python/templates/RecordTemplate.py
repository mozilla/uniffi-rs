...
class {{ type_name }}:
    ...
    {%- endfor %}):
        {%- for field in rec.fields() %}
        {%- let field_name = field.name()|var_name(config)%}
    ...
    def __str__(self):
        return "{{ type_name }}({% for field in rec.fields() %}{{ field.name()|var_name(config)}}={}{% if loop.last %}{% else %}, {% endif %}{% endfor %})".format({% for field in rec.fields() %}self.{{ field.name()|var_name(config)}}{% if loop.last %}{% else %}, {% endif %}{% endfor %})
...
class {{ ffi_converter_name }}(_UniffiConverterRustBuffer):
    ...
    def read(buf):
        return {{ type_name }}(
            {%- for field in rec.fields() %}
            {{ field.name()|var_name(config)}}={{ field|read_fn }}(buf),
            {%- endfor %}
        )
