...
{{ callable|def }} {{ callable.name }}({{ callable|arg_list }}){% if !callable.is_primary_constructor() %} -> {{ callable|return_type }}{% endif %}:
    ...
    _uniffi_return = await _uniffi_rust_call_async(
        _UniffiLib.{{ callable.ffi_func().name }}(
        ...
        _UniffiLib.{{ async_data.ffi_rust_future_poll.name }},
        _UniffiLib.{{ async_data.ffi_rust_future_complete.name }},
        _UniffiLib.{{ async_data.ffi_rust_future_free.name }},
    ...
    {%- else if let Some(return_type) = &callable.return_type().ty %}
