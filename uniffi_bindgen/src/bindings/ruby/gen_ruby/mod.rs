...
mod filters {
    ...
    pub fn coerce_rb(nm: &dyn fmt::Display, type_: &Type) -> Result<String, askama::Error> {
        Ok(match type_ {
            ...
            Type::Custom { .. } => panic!("No support for wrapped types, yet"),
        })
    }
    ...
    pub fn lower_rb(nm: &dyn fmt::Display, type_: &Type) -> Result<String, askama::Error> {
        Ok(match type_ {
            ...
            Type::Custom { .. } => panic!("No support for lowering wrapped types, yet"),
        })
    }
    ...
    pub fn lift_rb(nm: &dyn fmt::Display, type_: &Type) -> Result<String, askama::Error> {
        Ok(match type_ {
            ...
            Type::Custom { .. } => panic!("No support for lifting wrapped types, yet"),
        })
    }
}
