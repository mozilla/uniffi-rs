class RustBuffer < FFI::Struct
  ...
  def self.check_{{ canonical_type_name }}(v)
    {%- for field in rec.fields() %}
    {{ "v.{}"|format(field.name()|var_name_rb)|check_rb(field.as_type().borrow()) }}
    {%- endfor %}
  end
  ...
  def self.check_{{ canonical_type_name }}(v)
    ...
    if v.{{ variant.name()|var_name_rb }}?
      {%- for field in variant.fields() %}
        {{ "v.{}"|format(field.name())|check_rb(field.as_type().borrow()) }}
      {%- endfor %}
      return
    end
    {%- endfor %}
  ...
  def self.check_{{ canonical_type_name }}(v)
    v.each do |item|
      {{ "item"|check_rb(inner_type.borrow()) }}
    end
  end
