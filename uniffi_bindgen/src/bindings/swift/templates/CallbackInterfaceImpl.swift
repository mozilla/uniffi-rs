...
fileprivate let {{ callback_handler }} : ForeignCallback =
    { (handle: Int64, method: Int32, argsData: UnsafePointer<UInt8>, argsLen: Int32, out_buf: UnsafeMutablePointer<RustBuffer>) -> Int32 in
    ...
    switch method {
        ...
        case {{ loop.index }}:
            do {
                let cb = try {{ ffi_converter_name }}.slab.get(handle: handle)
