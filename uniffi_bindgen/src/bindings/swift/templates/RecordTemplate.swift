...
public struct {{ type_name }} {
    ...
    public {% if config.generate_immutable_records() %}let{% else %}var{% endif %} {{ field.name()|var_name(config) }}: {{ field|type_name(config) }}
    {%- endfor %}
...
extension {{ type_name }}: Equatable, Hashable {
    public static func ==(lhs: {{ type_name }}, rhs: {{ type_name }}) -> Bool {
        {%- for field in rec.fields() %}
        if lhs.{{ field.name()|var_name(config) }} != rhs.{{ field.name()|var_name(config) }} {
            return false
        }
        {%- endfor %}
        return true
    ...
    public func hash(into hasher: inout Hasher) {
        {%- for field in rec.fields() %}
        hasher.combine({{ field.name()|var_name(config) }})
        {%- endfor %}
    }
}
{% endif %}
...
public struct {{ ffi_converter_name }}: FfiConverterRustBuffer {
    ...
    public static func write(_ value: {{ type_name }}, into buf: inout [UInt8]) {
        {%- for field in rec.fields() %}
        {{ field|write_fn }}(value.{{ field.name()|var_name(config) }}, into: &buf)
        {%- endfor %}
    }
}
