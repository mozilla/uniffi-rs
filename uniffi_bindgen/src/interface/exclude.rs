...
pub fn exclude(ci: &mut ComponentInterface, exclude: &[String]) {
    let exclude: HashSet<&str> = exclude.iter().map(String::as_str).collect();

    remove_functions(&exclude, &mut ci.functions);
    remove_type_definitions(&exclude, &mut ci.records, &mut ci.types);
    remove_type_definitions(&exclude, &mut ci.enums, &mut ci.types);
    remove_type_definitions(&exclude, &mut ci.objects, &mut ci.types);
    remove_type_definitions(&exclude, &mut ci.callback_interfaces, &mut ci.types);

    for o in ci.objects.iter_mut() {
        remove_constructors(&exclude, &o.name, &mut o.constructors);
        remove_methods(&exclude, &o.name, &mut o.methods);
    }
    for rec in ci.records.iter_mut() {
        remove_constructors(&exclude, &rec.name, &mut rec.constructors);
        remove_methods(&exclude, &rec.name, &mut rec.methods);
    }
    for en in ci.enums.iter_mut() {
        remove_constructors(&exclude, &en.name, &mut en.constructors);
        remove_methods(&exclude, &en.name, &mut en.methods);
    }
    for cbi in ci.callback_interfaces.iter_mut() {
        remove_methods(&exclude, &cbi.name, &mut cbi.methods);
    }
}
