...
use super::{AsType, FfiDefinition, FfiTypeKind, TypeDefinition, TypeKind};
    ...
    fn dependency_names(&self, ffi_def: &FfiDefinition) -> Vec<String> {
        match ffi_def {
            FfiDefinition::Struct(ffi_struct) => ffi_struct
                .fields
                .iter()
                .filter_map(|f| Self::type_dependency_name(&f.ty.kind))
                .collect(),
            FfiDefinition::Function(func) => func
                ...
                .filter_map(|ty| Self::type_dependency_name(&ty.kind))
                .collect(),
...
impl FfiDefinitionDependencyLogic {
    fn type_dependency_name(ffi_type: &FfiTypeKind) -> Option<String> {
        match &ffi_type {
            FfiTypeKind::Struct(name) | FfiTypeKind::FunctionPointer(name) => Some(name.clone()),
            FfiTypeKind::Reference(inner) | FfiTypeKind::MutReference(inner) => {
                Self::type_dependency_name(&inner.kind)
            }
            _ => None,
        }
    }
...
impl DependencyLogic for TypeDefinitionDependencyLogic {
    ...
    fn item_name(&self, type_def: &TypeDefinition) -> String {
        type_def.as_type().canonical_name().to_string()
    }

    fn dependency_names(&self, type_def: &TypeDefinition) -> Vec<String> {
        match type_def {
            TypeDefinition::Builtin(ty) => match &ty.kind {
                TypeKind::Optional { inner_type } | TypeKind::Sequence { inner_type } => {
                    vec![inner_type.canonical_name()]
                }
                TypeKind::Map {
                    key_type,
                    value_type,
                } => {
                    vec![key_type.canonical_name(), value_type.canonical_name()]
                }
                _ => vec![],
            },
            ...
            TypeDefinition::Interface(i) => i
                ...
                .chain(i.methods.iter().flat_map(|m| {
                    m.arguments
                        .iter()
                        .map(|a| &a.ty)
                        .chain(&m.return_type.ty)
                        .chain(&m.throws_type.ty)
