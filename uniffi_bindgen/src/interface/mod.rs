...
impl ComponentInterface {
    ...
    pub fn ffi_names(&self) -> &FFINames {
        return self.ffi_names.as_ref().expect("FFINames not set yet");
    }
    ...
    pub fn ffi_rustbuffer_alloc(&self) -> FFIFunction {
        FFIFunction {
            name: self.ffi_names().uniffi_func("rustbuffer_alloc"),
    ...
    pub fn ffi_rustbuffer_from_bytes(&self) -> FFIFunction {
        FFIFunction {
            name: self.ffi_names().uniffi_func("rustbuffer_from_bytes"),
            arguments: vec![FFIArgument {
    ...
    pub fn ffi_rustbuffer_free(&self) -> FFIFunction {
        FFIFunction {
            name: self.ffi_names().uniffi_func("rustbuffer_free"),
            arguments: vec![FFIArgument {
                name: "buf".to_string(),
    ...
    pub fn ffi_rustbuffer_reserve(&self) -> FFIFunction {
        FFIFunction {
            name: self.ffi_names().uniffi_func("rustbuffer_reserve"),
            arguments: vec![
                FFIArgument {
                    name: "buf".to_string(),
                    type_: FFIType::RustBuffer,
    ...
    fn add_namespace_definition(&mut self, defn: Namespace) -> Result<()> {
        ...
        self.ffi_names = Some(FFINames::InterfaceNamespace(self.namespace.clone()));
        Ok(())
    }
    ...
    fn derive_ffi_funcs(&mut self) {
        // Need to clone ffi_names. We can't have a borrow live while mutating our components.
        let ffi_names = self.ffi_names().clone();
        for func in self.functions.iter_mut() {
            func.derive_ffi_func(&ffi_names);
        }
        for obj in self.objects.iter_mut() {
            obj.derive_ffi_funcs(&ffi_names);
        }
        for callback in self.callback_interfaces.iter_mut() {
            callback.derive_ffi_funcs(&ffi_names);
        }
    }
}
