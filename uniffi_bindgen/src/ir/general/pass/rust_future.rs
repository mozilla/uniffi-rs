...
pub fn step(module: &mut Module) -> Result<()> {
    ...
    // Change `callable.is_async` to `callable.async_data`
    module.visit_mut(|callable: &mut Callable| {
        callable.async_data = callable.is_async.then(|| {
            let return_type_name = match callable.ffi_return_type() {
    ...
    let all_async_return_types = [
        ...
        (Some(FfiType::Float32), "f32"),
        (Some(FfiType::Float64), "f64"),
        (Some(FfiType::RustArcPtr { module_name: "".into(), object_name: "".into(), }), "pointer"),
