...
impl FfiDefinitionDependencyLogic {
    fn type_dependency_name(ffi_type: &FfiType) -> Option<String> {
        match &ffi_type {
            FfiType::Struct(name) | FfiType::Function(name) => Some(name.clone()),
...
impl DependencyLogic for TypeDefinitionDependencyLogic {
    ...
    fn item_name(&self, type_def: &TypeDefinition) -> String {
        match type_def {
            ...
            | TypeDefinition::Custom(CustomType { self_type, .. }) => self_type.ty.canonical_name(),
        }
    }
    ...
    fn dependency_names(&self, type_def: &TypeDefinition) -> Vec<String> {
        match type_def {
            ...
            | TypeDefinition::Sequence(SequenceType { inner, .. }) => {
                vec![inner.ty.canonical_name()]
            }
            TypeDefinition::Map(MapType { key, value, .. }) => {
                vec![key.ty.canonical_name(), value.ty.canonical_name()]
            }
            TypeDefinition::Record(r) => {
                r.fields.iter().map(|f| f.ty.ty.canonical_name()).collect()
            }
            TypeDefinition::Enum(e) => e
                .variants
                .iter()
                .flat_map(|v| v.fields.iter().map(|f| f.ty.ty.canonical_name()))
                .collect(),
            ...
            TypeDefinition::Custom(custom) => {
                vec![custom.builtin.ty.canonical_name()]
            }
        }
    }
}
