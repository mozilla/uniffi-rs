...
pub trait Pipeline {
    ...
    fn execute(&mut self, initial: InitialRoot) -> Result<Self::Output> {
        self.execute_all_steps(initial, &mut NullPipelineStepRecorder)
    }
    ...
    fn print_steps(&mut self, input: InitialRoot, opts: PrintStepsOptions) -> Result<()> {
        let mut last_output: Option<(tempfile::TempPath, String)> = None;
        let mut step_recorder = PipelineCliStepRecorder::new(opts.clone());
        self.execute_all_steps(input, &mut step_recorder)?;
...
impl<PassRoot> Pass<PassRoot> {
    ...
    pub fn step(mut self, func: impl Fn(&mut PassRoot) -> Result<()> + 'static) -> Self {
        self.steps.push(Step::new(func));
        self
    }
...
impl<PassRoot> Step<PassRoot> {
    pub fn new(func: impl Fn(&mut PassRoot) -> Result<()> + 'static) -> Self {
        Self {
            name: std::any::type_name_of_val(&func),
            func: Box::new(func),
        }
    }
}
    ...
    NextRoot: Node + FromNode<PassRoot>,
{
    ...
    fn execute_all_steps(
        ...
        let prev_node = self.prev.execute_all_steps(initial, recorder)?;
...
impl Pipeline for InitialPipeline {
    ...
    fn execute_all_steps(
        ...
        recorder.record_step("initial", &initial);
        Ok(initial)
    }
}
