...
impl BindgenLoader {
        ...
        P: FnOnce(&Utf8Path, &[u8]) -> Result<Option<Vec<Metadata>>>,
    {
        if let Some(source_crate) = source_path.as_str().strip_prefix("src:") {
            let mut parser = match &self.target {
    ...
    pub fn load_cis(
        ...
        let mut ci_list = metadata
            ...
            .map(|group| {
                ...
                if self.is_src(source_path) {
                    ci.skip_checksum_checks = true;
                }
                Ok(ci)
            })
    ...
    pub fn load_pipeline_initial_root(
        ...
        if self.is_src(source_path) {
            root.checksum_mode = pipeline::initial::ChecksumMode::Skip;
        }
        Ok(root)
    }
    ...
    fn is_src(&self, source_path: &Utf8Path) -> bool {
        source_path.as_str().starts_with("src:")
    }
