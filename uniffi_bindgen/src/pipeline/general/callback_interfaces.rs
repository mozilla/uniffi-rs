...
pub fn pass(module: &mut Module) -> Result<()> {
    ...
    module.try_visit_mut(|cbi: &mut CallbackInterface| {
        cbi.try_visit_mut(|meth: &mut Method| {
            match &mut meth.callable.kind {
                CallableKind::Method { kind, .. } => {
                    *kind = MethodKind::CallbackInterface;
                }
    ...
    module.try_visit_mut(|int: &mut Interface| {
        if int.imp.has_callback_interface() {
            int.try_visit_mut(|meth: &mut Method| {
                match &mut meth.callable.kind {
                    CallableKind::Method { kind, .. } => {
                        *kind = MethodKind::TraitInterface;
                    }
                    kind => bail!("Unexpected callable kind: {kind:?}"),
                };
                Ok(())
            })?;
            let vtable = vtable(&crate_name, &int.name, int.methods.clone())?;
...
fn add_vtable_ffi_definitions(module: &mut Module) -> Result<()> {
    ...
    module.try_visit(|vtable: &VTable| {
        ...
        for (i, meth) in vtable.methods.iter().enumerate() {
            ...
            if seen_return_types.insert(ffi_return_type.clone()) {
                ffi_definitions.extend([
                    FfiStruct {
                        name: async_info.ffi_foreign_future_result.0.clone(),
                    ...
                    .into(),
                    FfiFunctionType {
                        name: async_info.ffi_foreign_future_complete.0.clone(),
                        arguments: vec![
                            FfiArgument::new(
                                "callback_data",
                            ...
                            FfiArgument::new(
                                "result",
                                FfiType::Struct(async_info.ffi_foreign_future_result.0.clone()),
                            ),
                        ],
        ...
        ffi_definitions.extend([
            FfiFunctionType {
                name: format!("CallbackInterfaceFree{module_name}_{interface_name}"),
            ...
            .into(),
            FfiStruct {
                name: format!("VTableCallbackInterface{}", interface_name),
                fields: vtable
                    .methods
                    .iter()
                    .map(|vtable_meth| {
    ...
    Ok(FfiFunctionType {
        ...
        .chain([
            FfiArgument {
                ...
                ty: FfiType::Function(async_data.ffi_foreign_future_complete.0.clone()),
            },
            FfiArgument {
