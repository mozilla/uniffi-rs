...
pub fn pass(module: &mut Module) -> Result<()> {
    ...
    module.visit_mut(|callable: &mut Callable| {
        ...
        let base_arguments = callable.arguments.iter().map(|arg| {
            FfiArgument! {
                name: arg.name.clone(),
        ...
        ffi_definitions.push(if callable.async_data.is_none() {
            FfiFunction! {
                name: ffi_func_name,
                async_data: None,
                arguments: receiver_argument.into_iter().chain(base_arguments).collect(),
            ...
            FfiFunction! {
                ...
                arguments: receiver_argument.into_iter().chain(base_arguments).collect(),
                return_type: FfiReturnType! { ty: Some(FfiType::Handle(HandleKind::RustFuture)) },
