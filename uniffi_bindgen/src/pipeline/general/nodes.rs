...
use uniffi_pipeline::{Node, NodeConstructMacros};
...
pub enum CallableKind {
    ...
    ///
    /// For trait interfaces this only applies to the Callables inside `vtable.methods` field.
    /// Callables inside `methods` will still have `Callable::Method`.
...
#[derive(Debug, Clone, PartialEq, Eq, Hash, Node, NodeConstructMacros)]
pub enum Type {
    // Primitive types.
    UInt8,
    Int8,
