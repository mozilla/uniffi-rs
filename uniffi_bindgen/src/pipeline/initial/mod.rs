...
#[derive(Debug, Default, Clone, Node)]
pub struct Root {
...
// names are changed.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Node)]
pub enum Metadata {
    ...
    pub fn from_library(
        ...
        for meta in macro_metadata::extract_from_library(path)? {
            match meta {
                ...
                meta => metadata.push(Metadata::try_from_node(meta)?),
            }
        }
        ...
        // Sort metadata to make things consistent
        root.metadata.sort();
        Ok(root)
    }
    ...
    fn add_metadata_from_udl(
        ...
        root.metadata.extend(
            metadata_group
                ...
                .map(|meta| meta.and_then(Metadata::try_from_node))
