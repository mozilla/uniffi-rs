...
use uniffi_bindgen::{Config as UniffiConfig, ForeignLanguageBackend, interface::ComponentInterface};
pub mod gen_kotlin;
...
pub fn run_main() -> anyhow::Result<()> {
    let matches = clap::App::new("uniffi-bindgen")
        ...
        .get_matches();
    if let ("generate", Some(m)) = matches.subcommand() {
        uniffi_bindgen::generate_custom_bindings::<KotlinBackend, &OsStr>(
