...
impl BindingGenerator for PythonBindingGenerator {
    ...
    fn write_bindings(
        ...
        if self.try_format_code {
            match Command::new("yaph").stdin(Stdio::piped()).stdout(Stdio::piped()).spawn() {
                Ok(mut child) => {
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    let binding_generator = PythonBindingGenerator::new(matches.is_present("stdout"), !matches.is_present("no_format"));
