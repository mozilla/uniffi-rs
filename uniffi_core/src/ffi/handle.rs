...
impl Handle {
    ...
    /// Get a clone of this handle to for pass-by-reference.
    ///
    /// Use this to pass a handle to the foreign side of the FFI, while also keeping ownership over
    /// it, and not calling `Arc::clone()`
    ///
    /// # Safety
    ///
    /// The foreign side must only use the handle while the call is in progress.
    pub unsafe fn clone_for_ref(&self) -> Self {
        <Handle as Clone>::clone(self)
    }
