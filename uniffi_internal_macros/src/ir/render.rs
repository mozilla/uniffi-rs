        ...
        if rest.is_some() {
            ...
            quote! {
                ...
                #path
                {
                    #fields
                    #trailing_comma
                    #(#missing_fields: ::uniffi_pipeline::Node::empty()),*
                }
            }
        }
    }
...
impl Node {
    ...
    pub fn render_empty(&self) -> TokenStream {
        let type_name = &self.ident;
        let body = match &self.def {
            NodeDef::Struct(st) => st.fields.construct_empty(&NodeKind::Struct { type_name }),
            NodeDef::Enum(variants) => match variants.first() {
                Some(v) => {
                    let variant_name = &v.ident;
                    v.fields.construct_empty(&NodeKind::Variant {
                        type_name,
                        variant_name,
                    })
                }
    ...
    pub fn construct_macros(&self) -> TokenStream {
        let type_name = &self.ident;
        match &self.def {
            NodeDef::Struct(st) => {
                let maybe_macro = st.fields.construct_macro(NodeKind::Struct { type_name });
                quote! { #maybe_macro }
            }
            NodeDef::Enum(e) => {
                let macros = e.iter().filter_map(|v| {
                    v.fields.construct_macro(NodeKind::Variant {
                        type_name,
                        variant_name: &v.ident,
                    })
                });
...
impl Fields {
    ...
    fn construct_empty(&self, kind: &NodeKind<'_>) -> TokenStream {
        match self {
            Self::Named(fields) => {
                let fields = fields.iter().map(|f| {
                    let name = &f.var_name;
                    quote! { #name: Node::empty() }
                });
                quote! {
                    #kind { #(#fields),* }
                }
            ...
            Self::Unnamed(fields) => {
                let fields = (0..fields.len()).map(|_| quote! { Node::empty() });
                quote! {
                    #kind (#(#fields),*)
                }
            }
    ...
    fn construct_macro(&self, kind: NodeKind<'_>) -> Option<TokenStream> {
        let name = match kind {
            NodeKind::Struct { type_name } => format_ident!("{type_name}"),
            NodeKind::Variant {
                type_name,
                variant_name,
            } => format_ident!("{type_name}_{variant_name}"),
        };
        let node_path = match kind {
            NodeKind::Struct { type_name } => quote! { #type_name },
            NodeKind::Variant {
                type_name,
                variant_name,
        ...
        match self {
            Self::Named(fields) => {
                ...
                Some(quote! {
                    ...
                    macro_rules! #name {
                        ($($tt:tt)*) => {
                            ::uniffi_pipeline::construct_node!(
                                [#(#fields),*]
                                #node_path {
                                    $($tt)*
                                }
