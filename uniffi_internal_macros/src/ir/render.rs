...
impl Node {
    pub fn render(&self, ir: Ir) -> TokenStream {
        ...
        match def {
            NodeDef::Struct(fields) => {
                ...
                quote! {
                    #attrs
                    #vis struct #ident #generics #fields #(#trailing_semi)*
                }
            }
    ...
    fn impl_from_node_for_pass_type(&self, input_module: &Path) -> TokenStream {
        ...
        let (impl_generics, from_ty_generics, into_ty_generics, where_clause) =
            self.generics.split_for_from_node_impl();
        ...
        quote! {
            ...
            impl #impl_generics #ir_mod::FromNode<#input_module::#type_name #from_ty_generics> for #type_name #into_ty_generics #where_clause {
                fn from_node(value: #input_module::#type_name #from_ty_generics) -> #ir_mod::Result<#type_name #into_ty_generics> {
                    #body
                }
            }
        }
    ...
    fn impl_from_node_for_output_type(&self, output_module: &Path) -> TokenStream {
        ...
        quote! {
            #[automatically_derived]
            impl #impl_generics #ir_mod::FromNode<#type_name #from_ty_generics> for #output_module::#type_name #into_ty_generics #where_clause {
                fn from_node(value: #type_name #from_ty_generics) -> #ir_mod::Result<#output_module::#type_name #into_ty_generics> {
                    #body
                }
            }
        }
...
impl Generics {
    fn render(&self) -> TokenStream {
        ...
        quote! { #lt_token #(#ty_params,)* #gt_token }
    }

    /// Generate tokens to handle the generic parts of `impl Node`
    ///
    /// This returns the tuple (impl_generics, ty_generics, where_clause).
    pub fn split_for_node_impl(&self) -> (TokenStream, TokenStream, TokenStream) {
        ...
        (
            quote! { <#(#types,)*> },
            quote! { <#(#types,)*> },
            quote! { where #(#types: #ir_mod::Node,)* #(#type_bounds)* },
        )
    }

    /// Generate tokens to handle the generic parts of `impl FromNode`
    ///
    /// This returns the tuple (impl_generics, from_ty_generics, into_ty_generics, where_clause).
    pub fn split_for_from_node_impl(&self) -> (TokenStream, TokenStream, TokenStream, TokenStream) {
        if self.lt_token.is_none() {
            return (
                TokenStream::default(),
                TokenStream::default(),
        ...
        let ir_mod = ir_mod();
        let mut from_ty_generics = vec![];
        let mut into_ty_generics = vec![];
        let mut where_clauses = vec![];

        for ty_param in self.ty_params.iter() {
            let ident = &ty_param.ident;
            let bounds = &ty_param.bounds;
            let from_ty = format_ident!("{ident}From");
            let into_ty = format_ident!("{ident}Into");
            where_clauses.push(quote! { #into_ty: #ir_mod::FromNode<#from_ty> });
            where_clauses.push(quote! { #from_ty: #bounds });
            where_clauses.push(quote! { #into_ty: #bounds });

            from_ty_generics.push(from_ty);
            into_ty_generics.push(into_ty);
        }

        (
            quote! { <#(#from_ty_generics,)* #(#into_ty_generics),*> },
            quote! { <#(#from_ty_generics,)*> },
            quote! { <#(#into_ty_generics,)*> },
