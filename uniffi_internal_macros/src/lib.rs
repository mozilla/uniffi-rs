...
#[proc_macro_derive(NodeConstructMacros, attributes(node))]
pub fn node_construct_macros(input: TokenStream) -> TokenStream {
    ir::expand_node_construct_macros(parse_macro_input!(input))
        .unwrap_or_else(syn::Error::into_compile_error)
        .into()
}
