...
pub struct CustomTypeOptions {
    into_bridge_type: Option<ConvertClosure>,
    try_from_bridge_type: Option<ConvertClosure>,
...
impl UniffiAttributeArgs for CustomTypeOptions {
    fn parse_one(input: ParseStream<'_>) -> syn::Result<Self> {
        ...
        if lookahead.peek(kw::into_bridge_type) {
            input.parse::<kw::into_bridge_type>()?;
            input.parse::<Token![:]>()?;
            Ok(Self {
                into_bridge_type: Some(input.parse()?),
                ..Self::default()
            })
        } else if lookahead.peek(kw::try_from_bridge_type) {
            input.parse::<kw::try_from_bridge_type>()?;
            input.parse::<Token![:]>()?;
            Ok(Self {
                try_from_bridge_type: Some(input.parse()?),
                ..Self::default()
            })
    ...
    fn merge(self, other: Self) -> syn::Result<Self> {
        Ok(Self {
            into_bridge_type: either_attribute_arg(self.into_bridge_type, other.into_bridge_type)?,
            try_from_bridge_type: either_attribute_arg(
                self.try_from_bridge_type,
                other.try_from_bridge_type,
            )?,
...
pub(crate) fn expand_custom_type(args: CustomTypeArgs) -> syn::Result<TokenStream> {
    ...
    let (into_bridge_type_param, into_bridge_type_expr) = match options.into_bridge_type {
    ...
    let (try_from_bridge_type, try_from_bridge_type_expr) = match options.try_from_bridge_type {
        Some(convert_closure) => convert_closure.token_tuple(),
        None => (
            quote! { val },
    ...
    Ok(quote! {
        ...
        #impl_spec {
            ...
            fn lower(#into_bridge_type_param: #custom_type ) -> Self::FfiType {
                <#uniffi_type as ::uniffi::Lower<crate::UniFfiTag>>::lower(#into_bridge_type_expr)
            }

            fn try_lift(v: Self::FfiType) -> ::uniffi::Result<#custom_type> {
                let #try_from_bridge_type = <#uniffi_type as ::uniffi::Lift<crate::UniFfiTag>>::try_lift(v)?;
                #try_from_bridge_type_expr
            }

            fn write(#into_bridge_type_param: #custom_type, buf: &mut Vec<u8>) {
                <#uniffi_type as ::uniffi::Lower<crate::UniFfiTag>>::write(#into_bridge_type_expr, buf);
            }

            fn try_read(buf: &mut &[u8]) -> ::uniffi::Result<#custom_type> {
                let #try_from_bridge_type = <#uniffi_type as ::uniffi::Lift<crate::UniFfiTag>>::try_read(buf)?;
                #try_from_bridge_type_expr
            }
...
pub(crate) fn expand_custom_newtype(args: CustomNewtypeArgs) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        uniffi::custom_type!(#ident, #uniffi_type, {
            into_bridge_type: |obj| obj.0,
            try_from_bridge_type: |val| Ok(#ident(val)),
        });
    })
}
