...
pub fn future_output_type(ty: &Type) -> Option<Type> {
    ...
    let path = args.args.iter().find_map(|arg| match arg {
        syn::GenericArgument::Type(Type::Path(second_path)) => Some(second_path),
        _ => None,
    })?;
    ...
    let trait_obj = args.args.iter().find_map(|arg| match arg {
        syn::GenericArgument::Type(Type::TraitObject(trait_obj)) => Some(trait_obj),
        _ => None,
    })?;
...
fn assoc_output(args: &syn::AngleBracketedGenericArguments) -> Option<Type> {
