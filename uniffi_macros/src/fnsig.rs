...
fn future_output_type(ty: &Type) -> Option<Type> {
    ...
    let Some(path) = args.args.iter().find_map(|arg| match arg {
    ...
    let Some(trait_obj) = args.args.iter().find_map(|arg| match arg {
...
mod tests {
    ...
    #[test]
    fn boxed_and_impl_futures_are_unwrapped() {
        ...
        assert_eq!(
            future_output("Box<dyn Future<Output = Arc<Self>>>"),
            None
        );
        // `impl Future` won't match either
        assert_eq!(
            future_output("impl Future<Output = i64>"),
            None
        );
    }
}
