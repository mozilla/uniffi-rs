...
///
/// This is a simple convenience macro to include the generated
/// UniFFI component scaffolding.
/// Use it like so:
///
/// ```rs
...
///
/// It will also generate metadata about the found UDL file, which will be
/// processed when the library is loaded to generate bindings.
#[proc_macro]
...
/// A helper macro to include component scaffolding generated by generate_namespaced_scaffolding
///
/// Use it like so:
///
/// ```rs
/// uniffi_macros::include_namespaced_scaffolding!("my_namespace");
/// ```
///
/// Like, [include_scaffolding!] but adds "namespace" metadata instead of UDL.
#[proc_macro]
pub fn include_namespaced_scaffolding(component_name: TokenStream) -> TokenStream {
    let name = syn::parse_macro_input!(component_name as LitStr);
    if std::env::var("OUT_DIR").is_err() {
        quote! {
            compile_error!("This macro assumes the crate has a build.rs script, but $OUT_DIR is not present");
