...
impl MetadataReader for &[u8] {
    ...
    fn read_type(&mut self) -> Result<Type> {
        self.read_optional_type()?
            .ok_or_else(|| anyhow::format_err!("saw TYPE_UNIT when a type was required"))
    }

    fn read_optional_type(&mut self) -> Result<Option<Type>> {
        let value = self.read_u8()?;
        Ok(match value {
            codes::TYPE_UNIT => None,
            v => Some(match v {
    ...
    fn read_func(&mut self) -> Result<FnMetadata> {
        Ok(FnMetadata {
            ...
            name: self.read_string()?,
            is_async: self.read_bool()?,
            inputs: self.read_inputs()?,
            return_type: self.read_optional_type()?,
            throws: self.read_optional_type()?,
        })
    }
