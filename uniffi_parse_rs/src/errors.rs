...
pub enum ErrorKind {
    ...
    InvalidDiscr,
    #[error("Can't find public path to item: {0}")]
    NoPubPath(String),
...
impl Error {
    ...
    pub fn context_from_err(self, e: Self) -> Self {
        if let Some((file_id, span)) = e.location {
            self.context(file_id, span, e.kind.to_string())
        } else {
            self
        }
    }
