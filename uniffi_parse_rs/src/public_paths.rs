...
impl<'ir> RPath<'ir> {
    ...
    pub fn public_path_to_item(
        ...
        match result {
            Ok(v) => Ok(v),
            Err(e) => {
                let name = match self.item().map(Item::name) {
                    Ok(n) => n,
                    Err(_) => "unknown".to_string(),
                };
                Err(Error::new_without_location(NoPubPath(name)).context_from_err(e))
            }
        }
    }
    ...
    fn _public_path_to_item(&self, ir: &'ir Ir, cache: &mut LookupCache<'ir>) -> Result<ItemNames> {
        trace!("Finding public path: {self}");
        let item = self.item()?;
        let module_path = self.parent()?;
...
mod test {
    ...
    fn test_find_public_paths() {
        ...
        assert_eq!(
            run_public_path_to_item(&ir, "public_paths::Url"),
            ItemNames {
                module_path: "public_paths".into(),
                name: "Url".into(),
                orig_name: None,
